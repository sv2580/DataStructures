...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::deepCopy()
	{
		...
		for (size_t i = 0; i < degree(); i++)//degre vrati pocet synov; i++)
		{
	...
	inline bool TreeNode<T>::isRoot()
	{
		...
		return false;
		//return parent_ == nullptr;skrateny zapis
	}
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		...
		{
			throw std::logic_error("TreeNode<T>::getBrother: Node is root, so it has no parent ");
		}
	}
	...
	{
		std::queue<TreeNode<T>*>* processsQueue = new std::queue<TreeNode<T>*>();
		processsQueue->push(current);
		while (!processsQueue->empty())
		{
			TreeNode<T>* processNode = processsQueue->pop();
			if (processNode != nullptr)
			{
				path_->push(processNode);
				for (size_t i = 0; i < current->degree(); i++)
				{
					processsQueue->push(current->getSon(i));
				}
			}
		}
		delete processsQueue;
	}
